...
NinjaNerd is an educational practice platform for **grades 1–6** in **English, Math and
...
legacy route: **grade ≤ 5** gets the base list of 5 per subject, **grade 6** the extended list
of 10.
...
- Validation is shape-exact (`hasOnly` + `hasAll`) with size caps: email ≤ 254, school_name
  ≤ 200, question ≤ 4000, user_answer ≤ 1000, subtopic ≤ 100, roll-up maps ≤ 18 keys.
...
`g<1-6>_<topic>`, incremented with `increment()` nested inside `set(…, {merge:true})` in the
...
**248 pass, 0 fail, 0 todo** across 22 suites. Every prompt/task ships with a unit or mock test
